...
@router.get('/crm-b2b/owner-candidates')
async def crm_owner_candidates(_: dict = Depends(require_admin)):
    from ..database import get_db

    db = get_db()
    query = {'status': 'active', 'role': {'': ['marketing', 'manager']}}
    projection = {
        '_id': 0,
        'username': 1,
        'firstName': 1,
    ...
    users = await db.users.find(query, projection).to_list(None)
    ...
    def _display_name(user: dict) -> str:
        ...
        return full or name or user.get('username', '')

    users.sort(key=lambda user: _display_name(user).lower())
