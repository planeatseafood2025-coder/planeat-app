async def _handle_postback(postback_data: str, user_id: str, reply_token: str, access_token: str) -> None:
    ...
    if action == "approve":
        from ..services.expense_service import approve_draft_dynamic
        await approve_draft_dynamic(draft_id, {"sub": manager_username})
        await db.line_approval_pending.delete_many({"draftId": draft_id})
        # query budget after approval (expense already moved to db.expenses)
        _ck = draft.get("catKey", "")
        _now = _dt.now()
        _agg = await db.expenses.aggregate([
            {"$match": {"date_iso": {"$regex": f"^{_now.strftime('%Y-%m')}"}, "catKey": _ck}},
            {"$group": {"_id": None, "total": {"$sum": "$amount"}}},
        ]).to_list(None)
        _spent = _agg[0]["total"] if _agg else 0.0
        _bdoc = await db.budgets.find_one({"monthYear": f"{_now.month:02d}/{_now.year}"}) or {}
        _mbudget = float(_bdoc.get("budgets", {}).get(_ck, {}).get("monthly", 0))
        print(f"[DEBUG postback] catKey={_ck!r} monthYear={_now.month:02d}/{_now.year} mbudget={_mbudget} spent={_spent}", flush=True)
        if recorder_uid:
            await _push_to_uid(recorder_uid, [_build_recorder_flex(
                draft, mode="approved", approver_name=manager_name,
                monthly_budget=_mbudget, spent_month=_spent,
            )])
        # ส่งลงกลุ่ม LINE OA
        try:
            from ..services.line_notify_service import notify_expense_approved
            await notify_expense_approved(draft, manager_username)
        except Exception as _ne:
            logger.warning("notify_expense_approved (postback) failed: %s", _ne)
        manager_confirm = {
...
import httpx
from fastapi import APIRouter, Request, Response, Path, Depends, HTTPException
...
async def _handle_approval_reply(text: str, user_id: str, reply_token: str, access_token: str) -> bool:
    ...
    if normalized in ("Y", "YES", "ใช่"):
        ...
        # query budget for approved card
        _ck2 = draft.get("catKey", "")
        _now2 = _dt.now()
        _agg2 = await db.expenses.aggregate([
            {"$match": {"date_iso": {"$regex": f"^{_now2.strftime('%Y-%m')}"}, "catKey": _ck2}},
            {"$group": {"_id": None, "total": {"$sum": "$amount"}}},
        ]).to_list(None)
        _spent2 = _agg2[0]["total"] if _agg2 else 0.0
        _bdoc2 = await db.budgets.find_one({"monthYear": f"{_now2.month:02d}/{_now2.year}"}) or {}
        _mbudget2 = float(_bdoc2.get("budgets", {}).get(_ck2, {}).get("monthly", 0))
        ...
        if recorder_user:
            ...
            if recorder_user.get("lineUid"):
                flex = _build_recorder_flex(draft, mode="approved", approver_name=manager_name,
                                            monthly_budget=_mbudget2, spent_month=_spent2)
        ...
        try:
            ...
            logger.warning("notify_expense_approved (Y/N) failed: %s", _ne2)
...
async def line_webhook(
    ...
    for event in events:
        ...
        if event_type == "join" and source_type == "group":
            ...
            # ส่ง Group ID ไปกลุ่มด้วย push (ไม่ต้องพึ่ง replyToken)
            try:
                logger.info("[JOIN] try block: gid=%s", updated_target_id)
                _db2 = get_db()
                _sys = await _db2.system_settings.find_one({"_id": SETTINGS_DOC_ID}) or {}
                _tok2 = _sys.get("mainLineOa", {}).get("token", "")
                logger.info("[JOIN] tok_len=%d tok_ok=%s", len(_tok2), bool(_tok2))
                if _tok2 and updated_target_id:
                    logger.info("[JOIN] sending push to group...")
                    _gid_msg = (
                        f"🤖 PlaNeat เชื่อมต่อกลุ่มแล้ว!\n\n"
                        f"📋 Group ID ของกลุ่มนี้:\n"
                        f"{updated_target_id}\n\n"
                        f"กรุณาคัดลอก Group ID นี้ ไปใส่ในหน้า\n"
                        f"⚙️ ตั้งค่า → ระบบควบคุมโมดูล\n"
                        f"เพื่อรับการแจ้งเตือนจากระบบ"
                    )
                    ...
                    async with _httpx.AsyncClient(timeout=5) as _hc:
                        await _hc.post(
                            ...
                            json={"to": updated_target_id, "messages": [{"type": "text", "text": _gid_msg}]},
                        )
            except Exception as _je:
                logger.warning("join group send failed: %s", _je)
            ...
            # ถ้าพิม @groupid หรือ "group id" → ตอบกลับ group ID
            if msg_text.lower() in ("@groupid", "groupid", "group id", "กรุ๊ปไอดี", "กลุ่มไอดี") and group_id:
                _rt2  = event.get("replyToken", "")
                _tok2 = conf.get("token", "")
                if _rt2 and _tok2:
                    await _send_reply(_rt2, f"📋 Group ID ของกลุ่มนี้:\n{group_id}", _tok2)
...
@router.post("/test/daily-summary/{username}")
async def test_daily_summary(username: str, current_user: dict = Depends(get_current_user)):
    """ส่ง preview สรุปรายวันไปที่ LINE ส่วนตัวของ user (admin เท่านั้น)"""
    if current_user.get("role") not in ("admin", "super_admin", "it_manager", "accounting_manager"):
        raise HTTPException(status_code=403, detail="ไม่มีสิทธิ์")
    db = get_db()
    user = await db.users.find_one({"username": username})
    if not user or not user.get("lineUid"):
        return {"success": False, "message": f"ไม่พบ lineUid ของ {username}"}
    from ..services.line_notify_service import preview_daily_summary_to_uid
    ok = await preview_daily_summary_to_uid(user["lineUid"])
    return {"success": ok, "sentTo": username, "lineUid": user["lineUid"]}


@router.get("/test/daily-preview")
async def test_daily_preview_by_name(name: str):
    """ส่ง preview แบบใหม่: ข้อความอนุมัติสั้น + สรุปวัน"""
    db = get_db()
    ...
    if not user:
        return {"success": False, "message": f"ไม่พบ user ที่ชื่อ '{name}'"}
    line_uid = user.get("lineUid", "")
    if not line_uid:
        return {"success": False, "message": f"พบ user '{user.get('username')}' แต่ยังไม่มี lineUid"}

    doc   = await db.system_settings.find_one({"_id": "system_settings"}) or {}
    token = doc.get("mainLineOa", {}).get("token", "")
    if not token:
        return {"success": False, "message": "ไม่พบ mainLineOa token"}

    from ..services.line_notify_service import preview_daily_summary_to_uid
    ok = await preview_daily_summary_to_uid(line_uid)
    full_name = f"{user.get('firstName','')} {user.get('lastName','')}".strip()
    return {"success": ok, "sentTo": full_name, "username": user.get("username"), "lineUid": line_uid}
